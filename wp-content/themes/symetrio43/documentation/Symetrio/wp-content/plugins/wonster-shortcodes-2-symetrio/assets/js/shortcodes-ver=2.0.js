		...
		wonster_slider : function()
		{
			WtrSliderClass.prototype = {
				...
				navigate : function( selfSlider, dir, index )
				{
					...
					var onEndAnimationCurrentItemNoAnimation = function() {
						$( selfSlider.items ).removeClass( 'current' );
						$( selfSlider.items ).removeClass( dir === 'next' ? 'navOutNext' : 'navOutPrev' );
						++cntAnims;
						if( cntAnims === 2 ) {
							selfSlider.isAnimating = false;
						}
					...
					var onEndAnimationNextItemNoAnimation = function() {
						$( selfSlider.items ).removeClass( 'current' );
						$( selfSlider.items ).removeClass( dir === 'next' ? 'navInNext' : 'navInPrev' );
						++cntAnims;
						if( cntAnims === 2 ) {
							selfSlider.isAnimating = false;
						}
					...
					if( support.animations ) {
						...
						onEndAnimationCurrentItemNoAnimation();
						onEndAnimationNextItemNoAnimation();
					}
