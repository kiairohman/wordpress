	...
	class WTR_Menu_Widget extends WP_Widget {
		...
		public function form( $instance ) {
			...
			<p>
				...
				<br>
				<small><?php _e( 'Page IDs, separated by commas.', 'wtr_framework' ); ?> </small>
			</p>
			<p>
				...
				<br>
				<small><?php _e( 'If nested menu is enabled', 'wtr_framework' ); ?> </small>
			</p>
			<?php
		}
...
class WTR_Walker_Page extends Walker_Page {
	...
	function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		if ( '' === $page->post_title ){
			$page->post_title = sprintf( __( '#%d (no title)', 'symetrio' ), $page->ID );
		}
