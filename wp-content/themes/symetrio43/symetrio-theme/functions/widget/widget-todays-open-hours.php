...
if( ! class_exists( 'WTR_Todays_Open_Hours_Widget' ) ) {
	...
	class WTR_Todays_Open_Hours_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			$opening_time_format	= ( 0 == $days[ $current_day ][1] ) ? '' : '<span>' . ( ( 1 == $days[ $current_day ][1] ) ? __( 'PM', 'symetrio' )  : __( 'AM', 'symetrio' ) ) . '</span>';
			$closing_time			= $days[ $current_day ][2];
			$closing_time_format	= ( 0 == $days[ $current_day ][3] ) ? '' : '<span>' .( ( 1 == $days[ $current_day ][3] ) ? __( 'PM', 'symetrio' )  : __( 'AM', 'symetrio' ) ). '</span>';
