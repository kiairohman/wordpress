...
if( ! class_exists( 'WTR_Widget_Tweets' ) ) {
	...
	class WTR_Widget_Tweets extends WP_Widget {
		...
		function widget($args, $instance)
		{
			...
			if( empty( $twitter_id ) OR empty( $consumer_key ) OR empty( $consumer_secret ) OR empty( $access_token ) OR empty( $access_token_secret ) OR empty( $count ) ) {
				if(  current_user_can('edit_theme_options') ){
					echo '<h2 style="font-family: HelveticaNeue,Helvetica,Arial,sans-serif; color: #5A95A7; font-size: 14px" >' . __( 'Fill all widget settings ', 'symetrio' ) .'<h2>';
				}
					...
					foreach( $tweets as $tweet ) {
						...
						echo '<li class="wtrWidgetTwitterStreamItem">';
							echo '<div class="wtrWidgetTwitterStreamItemTittle">';
								echo $tweetText;
							echo '</div>';
							echo '<span class="wtrWidgetTwitterStreamItemDate">' . human_time_diff( strtotime( $tweet->created_at ), current_time('timestamp') ) . ' ' .   __( 'Ago', 'symetrio' ) . ' </span>';
						echo '</li>';
					}
				echo '</ul>';
			}
		...
		{
			$defaults = array('title' => __( 'Recent Tweets', 'symetrio' ) , 'twitter_id' => '', 'count' => 3 );
