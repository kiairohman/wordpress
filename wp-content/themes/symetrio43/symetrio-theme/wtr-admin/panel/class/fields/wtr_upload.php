...
if ( ! class_exists( 'WTR_Upload' ) ) {
	...
	class WTR_Upload extends WTR_Filed {
					...
					<div class="wfSett <?php echo $mod_class; ?>">
						<div class="wonsterUpload">
							<div class="setCol-one-three">
								<a href="" class="WonButton blue fileSelect wtr_admin_file_upload" default_size="<?php echo $this->default_size; ?>" data-editor="<?php $this->id; ?>" target_type="<?php echo $this->target_type; ?>" title_modal="<?php echo esc_attr( $this->title_modal ); ?>" filter_content="<?php echo $this->filter_content; ?>"> <?php _e('Select file', 'wtr_framework' ) ?> </a>
							</div>
							<?php if( $this->info ) { ?>
		...
		?>
			<h3> <?php  _e( 'Avatar', 'wtr_framework' ) ?></h3>
